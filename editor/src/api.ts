...
export const exportIcons = (files: { name: string; svg: string }[], convert: boolean, build: boolean, colorsCss?: string) =>
  fetch('/api/export', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ files, convert, build, colorsCss }) }).then((r) => j<{ ok: true; written: string[]; convertOutput: string; buildOutput: string }>(r));
